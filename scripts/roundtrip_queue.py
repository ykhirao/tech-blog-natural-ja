...
#
# 第2弾では、頻度上位の和語のうち「英語に1対1で対応しなさそうな語」を足した。
# 第1弾で「作り」が生き残り48%と突出したので、同じ性質の語を探す。
# 「為る」「居る」のような機能語は測っても意味がないので入れない。
GROUPS = {
    ...
    # --- 第2弾 ---
    # 置き場所・保持。英語では put / place / hold / keep に散る
    "置く": ["置く", "配置", "設置", "格納", "保持", "持つ"],
    # 動作。work / run / move に散る
    "動く": ["動く", "動作", "稼働", "実行", "走る"],
    # 状態。state / status / condition に散る
    "状態": ["状態", "状況", "条件", "様子"],
    # 扱い。handle / deal / treat / manage に散る
    "扱う": ["扱う", "処理", "対応", "管理", "取り扱い"],
    # 見え方。look / appear / visible / show に散る
    "見える": ["見える", "表示", "見た目", "外観", "可視"],
    # 変化。change / vary / differ に散る
    "変わる": ["変わる", "変更", "変化", "差分", "異なる"],
}
