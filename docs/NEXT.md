...
**見出しの文体。** 本文とタイトルは測ったが、見出し(`## ...`)は測っていない。
`split_markdown()` が `headings` として分けているので、すぐ取れる。
タイトルが2024〜2025年に動いたので、見出しも別の時期に動いている可能性がある。

**コードブロックの量と言語。** ` ```python ` のような言語指定を数えれば、
何の言語が書かれているかの推移が出る。地の文が2倍になった一方で
コードがどうなったかは測っていない。
